...
export const swapEthToToken = async (tokenName: string , amount: string): Promise<void> =>{
  try{
...
export const swapTokenToEth = async (tokenName: string , amount: string): Promise<void> =>{
  try {
    const contractObj = await contract()
    if (!contractObj){
      console.error("Contract object is undefined")
...
export const swapTokenToToken= async (srcToken:string , destToken:string , amount:string): Promise<void> =>{
  try{
    const contractObj = await contract()
    if(!contractObj){
      console.error(error)
...
export const getTokenBalance = async (tokenName:string , address:string): Promise<void> =>{
  const contractObj = await contract()
  if(!contractObj){
    console.error(error)
    return
...
export const increaseAllowance = async (tokenName:string, amount:string): Promise<void>=>{
 try{
   const contractObj = await contract()
  if(!contractObj){
    console.error(error)
...
export const getAllHistory = async (): Promise<{ historyId: number, tokenA: string, tokenB: string, inputValue: string, outputValue: string, userAddress: string }[] | void> => {
  try {
    const contractObj = await contract();
    if (!contractObj) {
      console.error(error);
    ...
    const historyTransaction = getAllHistory.map((history: {
      ...
      outputValue: string,
      userAddress: string
    }): { historyId: number, tokenA: string, tokenB: string, inputValue: string, outputValue: string, userAddress: string } => ({
